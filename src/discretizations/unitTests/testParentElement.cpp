  ...
  for ( int a = 0; a < N; ++a )
  {
    for ( int b = 0; b < N; ++b )
    {
      for ( int c = 0; c < N; ++c )
      {
        ...
        EXPECT_NEAR( gradients[ 3 * (a * N * N + b * N + c) + 0 ], TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][0],
                     fabs( TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][0] * tolerance ) );
        EXPECT_NEAR( gradients[ 3 * (a * N * N + b * N + c) + 1 ], TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][1],
                     fabs( TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][1] * tolerance ) );
        EXPECT_NEAR( gradients[ 3 * (a * N * N + b * N + c) + 2 ], TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][2],
                     fabs( TEST_PARENT_ELEMENT_HELPER::referenceGradients[a][b][c][2] * tolerance ) );
      }
    }
  }
