 ...
 * @tparam SPACING_TYPE The spacing type to define the interpolation points
 * for the Lagrange polynomials.
 * @tparam USE_FOR_SEQUENCE If true, the staticFor will be used to calculate
 * values and gradient functions. If false, the executeSequence function will
 * be used.
 * 
...
public:
   ...
   * 
   * \f[ P_j(x) = \prod_{ { {k=0} \atop {k\neq j} } }^{n-1} \frac{x-x_k}{x_j-x_k} \f], where
   * x = @p coord
   */
...
private:
  /**
   * @brief Calculates the value of the Lagrange polynomial basis function
   * product factor at the given @p BF_INDEX at the input coordinate @p coord.
   * @tparam BF_INDEX The index of the basis function to calculate the value of.
