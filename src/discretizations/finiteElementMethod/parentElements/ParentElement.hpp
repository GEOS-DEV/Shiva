...
//  using IndexType = typename CellType::IndexType;
   ...
   * \f[ \Phi_{i_0 i_1 ... i_{(numDims-1)}}(\boldsymbol{\xi}) = \prod_{k=0}^{(numDims-1)} \phi_{i_k}(\xi_k)\text{, where }\\
   *      i_j \text{is index of the basis function in the jth dimension, and ranges from [0...(order+1)]}
   * \f]
   * 
   */
